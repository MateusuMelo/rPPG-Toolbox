...
* #### TRAIN / VALID / TEST / UNSUPERVISED DATA:
  ...
  * `BACKEND`: Select which backend to use for face detection. Currently, the options are HC (Haar Cascade), HCDLIB (Haar Cascade + ROI segmentation) or Y5F (YOLO5Face). We recommend using Haar Cascade (the config default) in order to reproduce results from the [NeurIPS 2023 Datasets and Benchmarks paper](https://arxiv.org/abs/2210.00716) that corresponds to this toolbox. If you want to use YOLO5Face in your own custom config, we recommend that you reference configs that use it as a default (e.g., FactorizePhys). If you wish to perform ROI-based evaluation (e.g., comparing different facial regions), use HCDLIB. This mode uses Haar Cascade for face detection and DLIB for landmark-based facial ROI segmentation.
